...
- (id)initWithCoordinate:(CLLocationCoordinate2D)coordinate andStation:(NSManagedObject*)_station{
	...
	if (self != nil) {
		...
		if([(NSNumber *) [_station valueForKey:@"longitude"] floatValue] >0.1f){
			//NSLog(@"turi coord");
			location.longitude = [(NSNumber *) [_station valueForKey:@"longitude"] floatValue];
		}
...
- (NSString *) title {
	...
	return [station valueForKey:@"firm"];
}

- (NSString *)subtitle {
	return [station valueForKey:@"address"];
